...
def create_market():
    ...
    try:
        ...
        market_dict = {
            key: value for key, value in market_dict.items() if not key.startswith("_")
        }
        s.add(newMarket)
        s.commit()
        ...
        s.rollback()
        return {"message": "error create market", "error": str(e)}, 500
    finally:
        s.close()
...
def markets_all():
    ...
    try:
        ...
        if len(markets) < 1:
            return {"message": "Market is empty"}, 404
...
def market_by_location():
    ...
    try:
        ...
        for row in get_market:
            markets.append({"location": row.location})
        if len(markets) < 1:
            return {"message": "Market is empty"}, 404
        return {"success": True, "data": markets}, 200
    except Exception as e:
...
def market_by_id(id):
    ...
    try:
        ...
        if market:
            seller = (
                s.query(Seller).filter(Seller.seller_id == market.seller_id).first()
            )
            if seller:
                return (
                    jsonify(
                        {
                            ...
                            "profile_picture": market.profile_picture,
                        }
                    ),
                    200,
                )
    ...
    try:
        ...
        if "profile_picture" in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                ext_name = os.path.splitext(filename)[1]
                timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
                new_filename = f"{current_user.user_id}-{timestamp}{ext_name}"
...
def delete_product(id):
    ...
    try:
        ...
        log_manager.set_before(before_data=market_str)
        log_manager.save()
        return jsonify({"success": True, "message": "Success delete market"}), 200
